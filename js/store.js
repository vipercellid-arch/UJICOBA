...
let siteSettings = { 
    ...
    // Konfigurasi API & Pembayaran V4
    dfUser: '', dfKey: '', dfActive: false,
    webhookUrl: '', botQrisActive: false,
    paymentExpired: 15 // Default 15 menit
};
...
let paymentTimerInterval = null; // Handler Timer QRIS
...
window.closeModal = (id) => {
    ...
    if(id === 'modal-payment') clearInterval(paymentTimerInterval);
}
...
// ==========================================
// PROSES PEMBELIAN (CHECKOUT & ANTI TABRAKAN)
// ==========================================
...
window.processDirectCheckout = async function() {
    ...
    // ==========================================
    // ALGORITMA ANTI-TABRAKAN KODE UNIK (COLLISION-FREE)
    // ==========================================
    if(paymentMethod === 'qris' && baseTotal > 0) {
        let isUnique = false;
        let attempts = 0;
        
        // Cek hingga 50 kali untuk menemukan slot kode unik yang kosong
        while(!isUnique && attempts < 50) {
            uniqueCode = Math.floor(Math.random() * 300) + 1;
            let tempTotal = baseTotal + uniqueCode;
            
            // Cek pada array orders global jika ada status UNPAID yang nominalnya sama
            const conflict = orders.some(o => o.status === 'UNPAID' && o.finalTotal === tempTotal);
            if(!conflict) {
                isUnique = true;
            }
            attempts++;
        ...
        if(!isUnique) {
            window.customAlert('Sistem Sibuk', 'Sistem sedang memproses banyak transaksi untuk nominal ini. Silakan coba 1-2 menit lagi setelah slot kosong.', 'warning');
            return;
        }
    }
...
// ==========================================
// TIMER KEDALUWARSA & QRIS UI (V4 MATURE)
// ==========================================
window.openQRISModal = function() {
    ...
    document.getElementById('payment-expired-ui').style.display = 'none';

    clearInterval(paymentTimerInterval); // Bersihkan Timer Lama
    ...
    // --- LOGIKA TIMER MUNDUR ---
    const orderData = orders.find(o => o.dbId === currentCheckoutSession.dbId) || currentCheckoutSession;
    const expiredMinutes = siteSettings.paymentExpired || 15;
    const orderTimeMs = new Date(orderData.date || Date.now()).getTime();
    const expireTimeMs = orderTimeMs + (expiredMinutes * 60 * 1000);
    
    const timerBox = document.getElementById('qris-timer-box');
    const timerDisplay = document.getElementById('qris-countdown-display');
    
    // Cek jika saat dibuka waktu sudah lewat atau status sudah EXPIRED
    if (Date.now() >= expireTimeMs || orderData.status === 'EXPIRED') {
        document.getElementById('qris-main-ui').style.display = 'none';
        document.getElementById('payment-expired-ui').style.display = 'block';
        if (orderData.status === 'UNPAID') updateDoc(doc(db, pathOrders, orderData.dbId), { status: 'EXPIRED' }).catch(()=>{});
    } else {
        timerBox.style.display = 'block';
        
        paymentTimerInterval = setInterval(async () => {
            const now = Date.now();
            ...
            if (diff <= 0) {
                ...
                // Pukul DB ke Expired untuk membebaskan Kode Unik
                const latestData = orders.find(o => o.dbId === currentCheckoutSession.dbId);
                if (latestData && latestData.status === 'UNPAID') {
                    await updateDoc(doc(db, pathOrders, currentCheckoutSession.dbId), { status: 'EXPIRED' }).catch(()=>{});
                }
            } else {
...
// ==========================================
// SISTEM CEK OTOMATIS PEMBAYARAN (SMART POLLING & AUTO-DELIVERY)
// ==========================================
...
window.startAutoCheckPayment = function() {
    ...
    // Batalkan pengecekan jika waktu payment sudah expired
    const latestOrder = orders.find(o => o.dbId === currentCheckoutSession.dbId);
    if(latestOrder && latestOrder.status === 'EXPIRED') return;
    ...
    let checkCount = 0;
    const maxChecks = 30; // Maksimal 30 detik pengecekan visual
    ...
    const checkInterval = setInterval(async () => {
        ...
        if (checkCount >= maxChecks) {
            clearInterval(checkInterval);
            try {
                // Jatuhkan fallback ke PENDING jika sistem butuh bantuan Admin (bukan expired)
                if(orderLatest && orderLatest.status === 'UNPAID') {
...
function setupFallbackWA(orderData, isAlreadySuccess) {
    ...
    if (isAlreadySuccess) {
        ...
        waText = `Halo Admin Vipercell, saya sudah transfer menggunakan QRIS untuk pesanan *${orderData.id}* senilai *Rp${orderData.finalTotal.toLocaleString('id-ID')}*. Namun sistem otomatis tidak merespon / gagal. Mohon diproses manual.`;
    }
...
window.goToPesananFromPay = function() {
    ...
    const orderIdToTrack = currentCheckoutSession.id;
    currentCheckoutSession = null;
...
window.trackOrder = function() {
    ...
    const sBadge = order.status === 'UNPAID' ? 'status-unpaid' : 
                   order.status === 'PENDING' ? 'status-pending' : 
                   (order.status === 'FAILED' || order.status === 'EXPIRED') ? 'status-failed' : 
                   'status-success';
                   
    const sName = order.status === 'UNPAID' ? 'Belum Dibayar' : 
                  order.status === 'PENDING' ? 'Menunggu Proses' : 
                  order.status === 'FAILED' ? 'Gagal/Batal' : 
                  order.status === 'EXPIRED' ? 'Kedaluwarsa' : 
                  'Sukses & Selesai';
...
window.renderUserOrders = function() {
    ...
    userOrders.forEach((o, index) => {
        const sBadge = o.status === 'UNPAID' ? 'status-unpaid' : 
                       o.status === 'PENDING' ? 'status-pending' : 
                       (o.status === 'FAILED' || o.status === 'EXPIRED') ? 'status-failed' : 
                       'status-success';
                       
        const sName = o.status === 'UNPAID' ? 'Belum Dibayar' : 
                      o.status === 'PENDING' ? 'Proses Antrian' : 
                      o.status === 'FAILED' ? 'Dibatalkan' : 
                      o.status === 'EXPIRED' ? 'Kedaluwarsa' : 
                      'Selesai';
...
window.renderAdminOrders = function() {
    ...
    filteredOrders.forEach(o => {
        const sBadge = o.status === 'UNPAID' ? `<span class="status-badge status-unpaid">UNPAID</span>` : 
                       o.status === 'PENDING' ? `<span class="status-badge status-pending">PENDING / ANTREAN</span>` : 
                       o.status === 'EXPIRED' ? `<span class="status-badge status-failed">EXPIRED</span>` :
                       o.status === 'FAILED' ? `<span class="status-badge status-failed">FAILED</span>` : 
        ...
        let actionBtn = '';
        if(o.status === 'PENDING' || (o.status === 'SUCCESS' && !o.adminReply)) {
            actionBtn = `<button class="btn btn-primary" style="padding:0.4rem 0.8rem; font-size:0.75rem;" onclick="window.promptProcessOrder('${o.dbId}')"><i class="fa-solid fa-bolt"></i> Proses</button>`;
        } else {
...
window.markOrderComplete = async function(statusType) {
    ...
    if(statusType === 'SUCCESS') window.customAlert('Sukses', 'Pesanan berhasil diproses.', 'success');
...
window.saveSettingsManual = async function() {
    ...
    const newSettings = {
        ...
        // API & Bot
        dfUser: document.getElementById('set-df-user') ? document.getElementById('set-df-user').value.trim() : '',
        dfKey: document.getElementById('set-df-key') ? document.getElementById('set-df-key').value.trim() : '',
        dfActive: document.getElementById('set-df-active') ? document.getElementById('set-df-active').checked : false,
        webhookUrl: document.getElementById('set-webhook-url') ? document.getElementById('set-webhook-url').value.trim() : '',
        botQrisActive: document.getElementById('set-bot-qris') ? document.getElementById('set-bot-qris').checked : false,
        paymentExpired: parseInt(document.getElementById('set-payment-expired')?.value) || 15
    };
...
window.populateAdminSettings = function() {
    ...
    if(document.getElementById('set-webhook-url')) document.getElementById('set-webhook-url').value = siteSettings.webhookUrl || '';
    const botQrisEl = document.getElementById('set-bot-qris');
    if(botQrisEl) botQrisEl.checked = siteSettings.botQrisActive || false;
    
    if(document.getElementById('set-payment-expired')) document.getElementById('set-payment-expired').value = siteSettings.paymentExpired || 15;
