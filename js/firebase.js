...
function listenData() {
    ...
    // Pengecekan real-time murni untuk mengupdate badge dan list pesanan
...
window.applySettingsToUI = function() {
    ...
    const waHref = `https://wa.me/${adminWaNum}?text=${encodeURIComponent('Halo Admin')}`;
...
// ==========================================
// SISTEM CEK OTOMATIS REALTIME (MATANG & KEBAL BUG)
// ==========================================
async function attemptClientAutoDelivery(orderData) {
    ...
    // Auto-delivery Super Kebal: Mengabaikan huruf besar/kecil dan spasi
    ...
    if (readyStock) {
        ...
        try {
            ...
            console.error("Gagal auto delivery:", e);
            return false;
        }
    } else {
        return false; 
...
window.startAutoCheckPayment = async function() {
    ...
    // Jika sistem mendeteksi pembayaran SUDAH SUKSES sebelum tombol loading ditekan
...
function setupFallbackWA(orderData, isAlreadySuccess) {
    ...
    let waText = '';
    if (isAlreadySuccess) {
        waText = `Halo Admin Vipercell, sistem menyatakan pembayaran untuk pesanan *${orderData.id}* BERHASIL senilai *Rp${orderData.finalTotal.toLocaleString('id-ID')}*. Namun stok otomatis sedang dalam pengisian. Mohon klaimkan pesanan saya segera.`;
    } else {
...
window.renderUserOrders = function() {
    ...
    userOrders.forEach((o, index) => {
        ...
        // Fitur Tutup Invoice: Pesanan selain UNPAID otomatis dilipat (collapsed)
...
window.adminAutoProcessOrder = async function(dbId) {
    ...
    window.openConfirm('ACC Otomatis', `Yakin verifikasi pembayaran dan langsung kirim stok otomatis untuk pesanan <b>${order.id}</b>?`, async (confirmed) => {
        if(!confirmed) return;
...
window.saveSettingsManual = async function() {
    ...
    const dfUserEl = document.getElementById('set-df-user');
    ...
    const newSettings = {
        ...
        dfActive: dfActiveEl ? dfActiveEl.checked : siteSettings.dfActive,
